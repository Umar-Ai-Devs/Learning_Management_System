...
interface Profile { name: string; email: string; roll_number: string; semester: number; department: string; phone: string; address: string; }
...
export default function StudentProfile() {
  ...
  if (!profile) return <ProtectedRoute role="student"><p style={{ color: '#64748b' }}>Loading...</p></ProtectedRoute>;
    ...
    <ProtectedRoute role="student">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, maxWidth: 860 }}>
        <Card>
          ...
          {[['Name', profile.name], ['Email', profile.email], ['Roll Number', profile.roll_number || '-'], ['Semester', profile.semester ? String(profile.semester) : '-'], ['Department', profile.department || '-'], ['Phone', profile.phone || '-'], ['Address', profile.address || '-']].map(([k, v]) => (
